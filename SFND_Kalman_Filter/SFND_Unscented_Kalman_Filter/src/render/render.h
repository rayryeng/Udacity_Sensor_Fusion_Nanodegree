...
struct Car {
  ...
  void render(pcl::visualization::PCLVisualizer::Ptr& viewer) {
    ...
    viewer->addCube(
        ...
        orientation, dimensions.x, dimensions.y, dimensions.z * 2 / 3, name);
    ...
    viewer->addCube(
        ...
        name + "frame");
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0, 0, 0, name + "frame");
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_REPRESENTATION,
    ...
    viewer->addCube(
        ...
        orientation, dimensions.x / 2, dimensions.y, dimensions.z * 1 / 3,
        name + "Top");
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, color.r, color.g, color.b,
        name + "Top");
    ...
    viewer->addCube(
        ...
        name + "Topframe");
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_COLOR, 0, 0, 0, name + "Topframe");
    viewer->setShapeRenderingProperties(
        pcl::visualization::PCL_VISUALIZER_REPRESENTATION,
        pcl::visualization::PCL_VISUALIZER_REPRESENTATION_WIREFRAME,
        name + "Topframe");
  ...
  void move(float dt, int time_us) {
    ...
    if (instructions.size() > 0 &&
        accuateIndex < (int)instructions.size() - 1) {
  ...
  bool checkCollision(Vect3 point) {
    ...
    double xPrime = ((point.x - position.x) * cosNegTheta -
                     (point.y - position.y) * sinNegTheta) +
                    position.x;
    double yPrime = ((point.y - position.y) * cosNegTheta +
                     (point.x - position.x) * sinNegTheta) +
                    position.y;
...
void renderHighway(double distancePos,
